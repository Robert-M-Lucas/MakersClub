...
#elif DEVICE_PID == 0xBC50 // simple mouse only has ir leds
                        #define LED_SETUP()		DDRB |= (1<<0); DDRB |= (1<<7); DDRB |= (1<<7);
		        #define L_LED_OFF()		PORTB &= ~(1<<0)
		        #define L_LED_ON()		PORTB |= (1<<0)
		        #define L_LED_TOGGLE()	PORTB ^= (1<<0)
#define TX_LED_OFF()	//PORTB |= (1<<7)
#define TX_LED_ON()		//PORTB &= ~(1<<7)
#define RX_LED_OFF()	//PORTC |= (1<<7)
#define RX_LED_ON()		//PORTC &= ~(1<<7)
                #else
			#define TX_LED_OFF()	PORTD |= (1<<5)
			#define TX_LED_ON()		PORTD &= ~(1<<5)
			#define RX_LED_OFF()	PORTB |= (1<<0)
